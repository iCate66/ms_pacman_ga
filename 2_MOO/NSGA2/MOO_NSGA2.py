...
GHOST_SCORE = 200
# LEVEL_COMPLETION_THRESHOLD = 10000  # Approximate score needed for first level
...
def run_nsga2(toolbox, pop_size=POPULATION_SIZE, generations=GENERATIONS, num_processes=NUM_PROCESSES):
    """Run NSGA-II optimisation with enhanced data collection"""
    random.seed(42)
    np.random.seed(42)
    ...
    # for i, ind in enumerate(population[:2]):  # Print first 2 individuals
    ...
    ## Debug score_objective
    # print("\nAfter initial evaluation:")
    # print("First 2 individuals from population:")
    # for i, ind in enumerate(population[:2]):
    #     print(f"Individual {i} fitness: {ind.fitness.values}")
    # print("\nFirst 2 values from gen_metrics['objective_values']:")
    # print(gen_metrics['objective_values'][:2])
  
    # When storing in generation_history
    # print("\nWhen storing in generation history:")
    # generation_metrics = {
    #     'generation': 0,
    #     'population_size': len(population),
    #     'objective_values': [ind.fitness.values for ind in population], 
    #     'metrics': gen_metrics.get('metrics', []),
    # }
    # print("First 2 values from generation_metrics['objective_values']:")
    # print(generation_metrics['objective_values'][:2])
    ...
    for gen in range(generations):
        ...
        # Store generation data with consistent structure
        generation_metrics = {
            'generation': gen,
            'population_size': len(population),
            'objective_values': [ind.fitness.values for ind in population],  # Store directly
            ...
            'objective_stats': {
                'mean': np.mean([ind.fitness.values for ind in population], axis=0),
                'std': np.std([ind.fitness.values for ind in population], axis=0),
                'max': np.max([ind.fitness.values for ind in population], axis=0),
                'min': np.min([ind.fitness.values for ind in population], axis=0)
            }
        }
        ...
        if (gen+1) % 100 == 0:
            ...
            with open(f'pacman_nsga2_checkpoint_gen_{gen+1}.pkl', 'wb') as cp_file:
                pickle.dump(checkpoint, cp_file)
  
    # Prepare final results with replay information
    pareto_front = tools.sortNondominated(population, len(population), first_front_only=True)[0]
  
    # Get best individuals by each objective
    ...
    replay_info = {
        ...
        'pareto_front': [(list(ind), ind.seed, ind.fitness.values, ind.metrics) for ind in pareto_front]
    }

    # Store comprehensive information for best performers
    performance_metrics = {
        'score_objective': {
            ...
            'best_total_score': max((ind.metrics['total_score'] if ind.metrics else 0) for ind in best_by_score)
        },
        'survival_objective': {
            'best_individuals': [(list(ind), ind.seed, ind.fitness.values, ind.metrics,
                                analyse_playstyle({'metrics': ind.metrics}))
        ...
        'pareto_front': [(list(ind), ind.seed, ind.fitness.values, ind.metrics,
                         analyse_playstyle({'metrics': ind.metrics}))
    ...
    ## score_objective debug
    # # Before saving pickle file, verify values
    # print("\nVerifying values before saving pickle:")
    # print("First generation objective values:")
    # first_gen = generation_history[0]
    # print(first_gen['objective_values'][:2])
    # print("\nLast generation objective values:")
    # last_gen = generation_history[-1]
    # print(last_gen['objective_values'][:2])
    ...
    results = {
        ...
        'config': {
            ...
            'LIMITED_ACTIONS': LIMITED_ACTIONS  # for agent replay
        }
    }
  
    # Save final results
    with open('pacman_nsga2_final_results.pkl', 'wb') as f:
        pickle.dump(results, f)
  
    return results

def main():
    logger.info("Starting NSGA-II optimisation for Ms. Pac-Man")
    logger.info(f"Generations: {GENERATIONS}")
    logger.info(f"Population size: {POPULATION_SIZE}")
    logger.info(f"Number of processes: {NUM_PROCESSES}")
   
    toolbox = setup_nsga2()
    results = run_nsga2(toolbox)
    ...
    logger.info(f"Optimisation complete - Results saved to pacman_nsga2_final_results.pkl")
